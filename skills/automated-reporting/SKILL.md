...
**Session summary cron:** `3e98de640cdb` (8 AM UK, qwen3.6-27b on LM Studio — switched from OpenRouter due to generic "ERROR" responses). Writes daily session summaries to `~/.obsidian_vault/Session Summaries/`.

**Daily brief cron:** `f8c7b071db07` (8 AM UK, nvidia/nemotron-3-nano-4b). Delivers to Telegram + origin chat. Server timezone is CEST (UTC+2) so cron fires at 09:00 server time = 08:00 UK time year-round (UK and Germany share DST boundaries).

**Cron receipt (proof of receipt):** `f6cdc8ee995c` (8:05 AM daily, runs 5 min after daily brief). Status digest of all cron jobs — confirms which ran successfully, which failed. Script: `~/.hermes/scripts/cron-receipt.py`.
...
- **Nano model ID was wrong.** Cron had `qwen/nemotron-nano-4b` but LM Studio uses `nvidia/nemotron-3-nano-4b`. Wrong ID causes HTTP 400, not a session_search crash. Nano works fine for cron once the ID is correct.
- **Cron prompt drift.** The daily brief cron prompt (`f8c7b071db07`) must be updated when format rules change — the skill alone doesn't control cron output.
