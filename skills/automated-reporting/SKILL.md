...
Generate concise automated reports, briefings, and status updates for {{USER}}.
...
{{USER}} wants briefs detailed enough to be useful but still skimmable. May 28 was 38 lines (too verbose) — then nemotron condensed too much (too brief). Sweet spot: ~25-30 lines with brief context per item.
...
4. If vault is offline ({{DESKTOP_HOST}} down), file stays in pending dir
...
**Session summary cron:** `{{CRON_ID_SESSION_SUMMARY}}` (8 AM UK, qwen3.6-27b on {{LMS}} — switched from OpenRouter due to generic "ERROR" responses). Writes daily session summaries to `~/.obsidian_vault/Session Summaries/`.

**Daily brief cron:** `{{CRON_ID_DAILY_BRIEF}}` (8 AM UK, nvidia/nemotron-3-nano-4b). Delivers to Telegram + origin chat. Server timezone is CEST (UTC+2) so cron fires at 09:00 server time = 08:00 UK time year-round (UK and Germany share DST boundaries).

**Cron receipt (proof of receipt):** `{{CRON_ID_CRON_RECEIPT}}` (8:05 AM daily, runs 5 min after daily brief). Status digest of all cron jobs — confirms which ran successfully, which failed. Script: `~/.hermes/scripts/cron-receipt.py`.
...
- **Cron prompt drift.** The daily brief cron prompt (`{{CRON_ID_DAILY_BRIEF}}`) must be updated when format rules change — the skill alone doesn't control cron output.
- **User wants bold on section labels.** {{USER}} requested bold headings (e.g. **🔑 TODAY**) — applied May 27.
- **Timezone is UK (BST/GMT).** {{USER}} is in UK, not Europe/CEST. Cron jobs fire at 09:00 server time = 08:00 UK time year-round (UK and Germany share DST boundaries).
...
- **Personal context files.** `~/.obsidian_vault/Atlas/IDENTITY.md` and `SOUL.md` are source of truth for {{USER}}'s values, decision framework, and identity. Re-read these files periodically to catch user updates. `USER.md` in Hermes memory is a distilled summary — vault files always win on conflicts.
