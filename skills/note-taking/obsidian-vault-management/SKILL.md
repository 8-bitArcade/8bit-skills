...
**When**: Vault lives on user's Windows PC (`100.110.93.103`), agent runs on VPS. This is the recommended approach for cross-machine vault access.
...
sshfs 123@100.110.93.103:"C:/Users/123/Documents/AI_Vault/Admin" ~/.obsidian_vault -o allow_other,reconnect,_netdev
...
- ⚠️ Vault path on Windows: user's username is `123`, vault at `C:\Users\123\Documents\AI_Vault\Admin`.
...
```bash
scp ~/.obsidian_vault/source/file.md 123@100.110.93.103:"C:/Target/Vault/Subfolder/"
```
...
- **Windows PC (vault host):** `100.110.93.103` — also runs LM Studio
- **VPS (agent host):** `100.110.135.127` (user: russell, Ubuntu)
...
ssh -L 3000:localhost:3000 russell@100.110.93.103 -N -f
```
...
```bash
# On VPS:
sshfs russell@100.110.93.103:/path/to/vault /mnt/obsidian
