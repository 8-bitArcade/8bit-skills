...
**Pattern:** Assign lightweight models (nemotron-nano-4b) to cron jobs that do data gathering, formatting, or simple reporting. Reserve larger models (qwen3.6-27b) for reasoning-heavy tasks.

```python
# Via cronjob API — the CORRECT way
cronjob(action='update', job_id='...', model={'provider': 'lmstudio', 'model': 'qwen/nemotron-nano-4b'})
```
