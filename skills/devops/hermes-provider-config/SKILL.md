...
- User asks to configure a new provider (OpenRouter, {{LMS}}, etc.)
...
- **Cron jobs pin models at creation** — cron jobs do NOT use `fallback_model`. They pin the provider+model specified at creation time. If {{LMS}} goes down, a cron job pinned to {{LMS}} will fail even though fallback is configured globally. Fix: set `model: {provider: openrouter, model: "openrouter/owl-alpha"}` when creating cron jobs that need reliability, or use `cronjob action=update` to repin after outages.
...
### Problem
Hermes does NOT dynamically discover models from {{LMS}}'s `/v1/models` endpoint for the slash command dropdown. The model list comes from a predefined catalog configured in `model_catalog.providers`.

### Solution
Add your {{LMS}} models to the custom provider section:
...
2. **Update config.yaml** (protected file — use Python or CLI):
   ...
   **Option B: Using CLI:**
   ```bash
   hermes config set model_catalog.providers.lmstudio.base_url "http://{{LMS_HOST}}/v1"
...
- **Model ID format must match {{LMS}} API** — use exact IDs from `/v1/models` response (e.g., `{{MODEL_LARGE}}`, not just `qwen3.6-27b`).
...
- `references/lmstudio-model-catalog.md` — {{LMS}} model catalog configuration guide
