...
```
Telegram/Discord → VPS Hermes → Workstation (LM Studio, primary, tier 1)
...
The health endpoint runs on the **VPS** (not the workstation). It polls the workstation via Tailscale status and `curl` to LM Studio. Returns two views:

- `/health` — full diagnostics (workstation tailscale + LM Studio + VPS disk/memory/load)
...
3. **Tier 2:** Workstation offline → look up phone in Tailscale (`tailscale status --json`, match hostname containing "nubia"/"z70" or OS == "android")
...
4. **Tier 3:** Both local + mobile unreachable → call OpenRouter API (`https://openrouter.ai/api/v1/chat/completions`, model `openrouter/auto`)
   - Reads API key from `~/.hermes/config.yaml` if `OPENROUTER_API_KEY` env var is not set
...
- **Tailscale IP changes** — mobile uses DHCP within Tailscale network. Cron relay resolves IP dynamically from `tailscale status --json`, never hardcode
- **Workstation offline ≠ sleep** — if workstation is truly off Tailscale (not just asleep), last_seen timestamp will be hours old. Use Tailscale `online` field, not just `last_seen`
- **Model GGUFs are large** — download over WiFi, not mobile data. Use a download script that validates SHA256.
- **OpenRouter key not in VPS env** — Hermes config has the key but standalone Python scripts can't see Hermes env. The `cron_relay.py` script reads the key from `~/.hermes/config.yaml` directly. Don't rely on `OPENROUTER_API_KEY` env var on the VPS.
...
4. After phone joins Tailscale: `tailscale status --json` shows phone hostname/IP
...
- `hermes-provider-config` — model catalog, LM Studio configuration