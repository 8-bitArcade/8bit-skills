...
description: "Troubleshoot LM Studio model catalog issues in Hermes — dropdown not showing models, YAML errors, gateway caching."
version: 1.0.0
author: Russell
license: MIT
---
...
# Verify LM Studio provider section exists
grep -A5 'model_catalog:' ~/.hermes/config.yaml | grep -A4 'lmstudio'

# Test direct connection to LM Studio
curl http://100.110.93.103:1235/v1/models | python3 -c "import sys,json; print(f'{len(json.load(sys.stdin)[\"data\"])} models found')"
...
- Your custom LM Studio models missing
...
Must match LM Studio API `/v1/models` response exactly.

**Correct formats:**
- `qwen/qwen3.6-27b` (with slash prefix)
- `qwen3.5-9b` (no prefix)  
- `mistralai/mistral-7b-instruct-v0.3`

**Incorrect:**
- `qwen3.6-27b` when LM Studio returns `qwen/qwen3.6-27b`
