...
description: "Configure LM Studio model catalog in Hermes — add custom models to slash command dropdown."
version: 1.0.0
author: Russell
license: MIT
---
...
- Need to add custom LM Studio models to available list
- "Why can't I select my qwen3-coder-30b model?"

## Problem

Hermes does NOT dynamically discover models from LM Studio's `/v1/models` endpoint. The slash command dropdown uses a predefined catalog configured in `model_catalog.providers`.

## Solution

Add your LM Studio models to the custom provider section in config.yaml:
...
```bash
curl http://100.110.93.103:1235/v1/models | python3 -c "import sys,json; [print(f'  - {m[\"id\"]}') for m in json.load(sys.stdin)['data']]"
```
...
- **Model ID format must match LM Studio API exactly** — use exact IDs from `/v1/models` response (e.g., `qwen/qwen3.6-27b`, not just `qwen3.6-27b`).
