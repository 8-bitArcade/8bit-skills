...
- **Telegram cron jobs cannot use interactive buttons** — Hermes' Telegram adapter intercepts ALL callback queries. Only callbacks with prefixes `ea:`, `sc:`, `cl:`, `mp:`, `gt:` are processed. Custom callback data (e.g., `approve:skill-name`) is eaten by the catch-all and discarded. Cron jobs run autonomously with no user present, so the `clarify` tool (which renders native Telegram buttons) cannot be used either. For Telegram cron jobs: send text messages only, no inline keyboards.
