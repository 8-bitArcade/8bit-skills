...
- **Strict Chronology:** Always maintain a strict chronological timeline. Do not jump ahead in time. Cover cloud tools and orchestration (n8n, Make.com, CrewAI) *before* discussing local migration or local harnesses (OpenClaw, NanoClaw, Hermes).
...
- Requires all 4 creds: `TWITTER_API_KEY`, `TWITTER_API_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_TOKEN_SECRET`
...
**Setup:** Complete. Instagram Business account (`@8bit_arcade1`) linked to Facebook Page (8Bit). Long-lived token stored in `.env`.
...
- Token stored as `META_LONG_LIVED_TOKEN` in `.env`
...
- **Model selection for drafts.** Use qwen3.6-27b for draft generation — nano model produces too minimal output, qwen3.5-9b times out with longer prompts. 27b is the sweet spot for quality + reliability.
