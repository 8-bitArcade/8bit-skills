...
description: Provider configuration, fallback chains, and troubleshooting for Hermes LLM providers (OpenRouter, LM Studio, etc.)
category: software-development
trigger: config, provider, fallback, API key, OpenRouter, LM Studio, provider not working, fallback not triggering
---
...
2. `.env` keys must be uncommented — `# OPENROUTER_API_KEY=*** is silently ignored
...
grep 'OPENROUTER_API_KEY' ~/.hermes/.env | grep -v '^#'
...
- Models must be loaded in LM Studio UI before they appear available
